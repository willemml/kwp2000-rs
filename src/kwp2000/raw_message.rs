...
impl RawMessage {
    ...
    pub fn new_query(service: ServiceId, data: Vec<u8>) -> Self {
        Self::new_query_addressed(AddressMode::None, None, None, service, data)
    }
    pub fn addressed(mut self, target: u8, source: u8) -> Self {
        self.target = Some(target);
        self.source = Some(source);
        self.mode = AddressMode::Physical;
        self
    }
    pub fn new_query_addressed(
        mode: AddressMode,
        target: Option<u8>,
        source: Option<u8>,
        service: ServiceId,
    ...
    pub fn from_bytes<R: Read>(source: &mut R) -> Result<Self, Error> {
        ...
        let data = if length > 1 {
            ...
            source.read_exact(dbuf).map_or_else(
                |e| {
